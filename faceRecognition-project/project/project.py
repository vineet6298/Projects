...
def recog():
    ...
    while True:  #This is a forever loop
        ...
        if cv2.waitKey(10) == ord('q'):
            op = pyautogui.confirm("Close the Program 'Spyeye'?") 
            if op == 'OK':
                print("Out")
                break
...
def data_Train():
    ...
    Enter User ID.\n\nnote: numeric data only.""", title='spyeye', default='none')
    #check for user input
    
    """
    if id >  :
        print(id)
        pyautogui.alert(text='WRONG INPUT',title='spyeye',button='Back')
        recog()
    """
...
#Options checking
opt =pyautogui.confirm(text= 'Chose an option', title='Spyeye', buttons=['START', 'Train', 'Exit'])
if opt == 'START':
...
if opt == 'Train':
    ...
    \nClick 'Ready' when you're ready.""", title='spyeye', buttons=['Ready', 'Cancel'])
